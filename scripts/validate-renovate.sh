...
DOCKER_CMD="docker run --rm -v ${CONFIG_FILE}:/usr/src/app/renovate.json:ro"

# Add strict flag if enabled
if [[ "${STRICT_MODE}" == "true" ]]; then
    VALIDATOR_CMD="renovate/renovate:${RENOVATE_VERSION} renovate-config-validator --strict"
    echo "  - Running in strict mode (will fail if config needs migration)"
else
    VALIDATOR_CMD="renovate/renovate:${RENOVATE_VERSION} renovate-config-validator"
fi

echo ""
echo "Command: ${DOCKER_CMD} ${VALIDATOR_CMD}"
echo ""

# Run the validator and capture output
if ${DOCKER_CMD} ${VALIDATOR_CMD}; then
    echo ""
